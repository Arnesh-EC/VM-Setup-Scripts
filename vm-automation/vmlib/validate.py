...
def validate_ipv4(value: str) -> str:
    ...
    if len(parts) != 4:
        raise argparse.ArgumentTypeError(
            f"Invalid IPv4 address '{value}': expected dotted-quad format."
        )
    for part in parts:
        try:
            n = int(part)
        ...
        if not 0 <= n <= 255:
            raise argparse.ArgumentTypeError(
                f"Invalid IPv4 address '{value}': octet '{part}' out of range 0–255."
            )
    return value
...
def validate_prefix(value: str) -> int:
    try:
        ...
        raise argparse.ArgumentTypeError(
            f"Invalid prefix '{value}': must be an integer."
        )
    if not 1 <= n <= 32:
        raise argparse.ArgumentTypeError(
            f"Invalid prefix '{value}': must be between 1 and 32."
        )
    return n
